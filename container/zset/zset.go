...
func (ss *SortedSet) Load(rd io.Reader) error {
	...
	ascending, err := util.ConvBool(head[0])
	if err != nil {
		return fmt.Errorf("zset parse ascending fail")
	}

	countmax, err := util.ConvInt64(head[1])
	if err != nil {
		return fmt.Errorf("zset parse countmax fail")
	}

	count, err := util.ConvUint64(head[2])
	if err != nil {
	...
	for i := uint64(0); i < count; i++ {
		...
		tokens := strings.SplitN(string(line), ",", 2)
		if len(tokens) != 2 {
			return fmt.Errorf("zset parse data fail:%+v", line)
		}

		score, err := util.ConvInt64(tokens[0])
		if err != nil {
