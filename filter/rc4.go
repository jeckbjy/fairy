...
func NewRC4(key string, useConnKey bool) fairy.Filter {
	f := &zRC4Filter{useConnKey: useConnKey}
	if key != "" {
		f.key = []byte(key)
	}
...
type zRC4Filter struct {
	base.BaseFilter
	key        []byte // default key
	useConnKey bool
}

func (rf *zRC4Filter) newCipher(conn fairy.Conn) (*rc4.Cipher, error) {
	key := rf.key
...
func (rf *zRC4Filter) handle(ctx fairy.FilterContext) fairy.FilterAction {
...
func (rf *zRC4Filter) HandleRead(ctx fairy.FilterContext) fairy.FilterAction {
	return rf.handle(ctx)
}

func (rf *zRC4Filter) HandleWrite(ctx fairy.FilterContext) fairy.FilterAction {
	return rf.handle(ctx)
}
